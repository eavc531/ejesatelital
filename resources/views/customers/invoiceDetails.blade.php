...
@section('htmlheader_title')
   Factura Nro: {{str_pad($invoices->number, 3, "0", STR_PAD_LEFT)}}
@endsection
...
@section('main-content')
   ...
   @if(isset($invoices->stateSend) and $invoices->stateSend == 'Pago Realizado' and Auth::user()->role != 'Cliente')
            ...
            <div class="col-sm-6">
               <table class="table table-sm">
                  ...
                  <tr>
                     ...
                     <th>Mes(es): {{$invoices->invoiceperiod}}
                     </th>
                  </tr>
                  <tr>
               ...
               <tbody>
                  @foreach ($invoiceDetails as $id)
                     <tr>
                        ...
                        @if($id->type == 'Descuento' and auth::user()->role == 'Administrador')
                        <td class="unit">-{{$id->payment}}
                        ...
                        <input type="hidden" name="" value="{{$sum-=$id->payment}}">
                        @elseif($id->type == 'Adicional' and auth::user()->role == 'Administrador')
                        <td class="unit">{{$id->payment}}
                           <a href="{{route('deleteDF',$id->id)}}" onclick="return confirm('¿Esta Segur@ de Eliminar esta fila de la Factura?')" class="pull-right"><span aria-hidden="true" style="color:red">&times;</span></a>
                        </td>
                        <input type="hidden" name="" value="{{$sum+=$id->payment}}">
